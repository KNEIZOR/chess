document.addEventListener("DOMContentLoaded", () => {
    ...
    const toggleCarouselButtons = () => {
        const membersCarouselBtn = document.querySelector(
            ".members__carousel-btn"
        );
        const carouselBtnMobile = document.querySelector(
            ".carousel-btn--mobile"
        );
    ...
    const createCarousel = ({
        ...
        const updateCounter = () => {
            ...
            numberCarusel.textContent =
                visibleSlides > 1 ? `${start}-${end}` : start;
        };
